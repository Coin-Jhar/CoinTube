...
def get_user_input(use_template=False):  # Add use_template parameter
    ...
    output_dir = input(
        f"Enter the output directory (default: {default_dir}): ") or default_dir

    # Validate output directory path
    while not os.path.isdir(
            output_dir) or not os.access(output_dir, os.W_OK):
        print(
            "Invalid output directory or no write permissions. Please try again."
        )
        output_dir = input(
            f"Enter the output directory (default: {default_dir}): ") or default_dir
    ...
    if not use_template:  # Only prompt if not using a template
        ...
        quality = input(
            "Enter desired quality (best/high/medium/low, default: best): "
        ) or "best"
        ...
        format = input(
            "Enter desired format (mp4/webm/mp3, default: mp4): ") or "mp4"
        ...
        download_subs = input(
            "Download subtitles? (yes/no, default: no): ").lower() or "no"
    ...
    if is_playlist:
        playlist_selection = input(
            "Select specific videos? (yes/no, default no): ").lower() or "no"
        if playlist_selection == "yes":
            while True:
                try:
                    ...
                    print(
                        "Invalid input. Please enter comma-separated numbers or ranges."
                    )
